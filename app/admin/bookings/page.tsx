...
import {
    Loader2, Search, Eye, Calendar, TrendingUp,
    Clock, ChevronLeft, ChevronRight, Building2, BedDouble,
    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto">
            ...
            <div>
                ...
                <p className="text-[var(--muted-text)]">Manage all bookings across the platform.</p>
            </div>
            ...
            <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
                ...
                <Card className="bg-[var(--card-bg)] border-white/10 text-[var(--page-text)]">
                    <CardContent className="p-4 flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-2xl font-bold text-gold-500">{stats.revenue.toFixed(2)} DH</p>
                            <p className="text-xs text-[var(--muted-text)]">Total Volume</p>
                        </div>
                    </CardContent>
                </Card>
                <Card className="bg-[var(--card-bg)] border-white/10 text-[var(--page-text)]">
                    <CardContent className="p-4 flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-[var(--muted-text)]">Active / Pending</p>
                        </div>
                    </CardContent>
                </Card>
                <Card className="bg-[var(--card-bg)] border-white/10 text-[var(--page-text)]">
                    <CardContent className="p-4 flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-[var(--muted-text)]">Cancelled</p>
                        </div>
                    </CardContent>
                </Card>
            </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-4 justify-between">
                        <div className="relative w-full md:w-96">
                            ...
                            <Input
                                ...
                                className="pl-10 bg-[var(--surface-100)] border-white/10"
                                value={search}
                                onChange={(e) => setSearch(e.target.value)}
                            />
                        </div>
                        <Tabs defaultValue="all" onValueChange={setStatusFilter} className="w-full md:w-auto">
                            <TabsList className="bg-[var(--surface-100)] border border-white/10">
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="border-white/10 hover:bg-white/5">
                                ...
                                <TableHead className="text-[var(--muted-text)]">Host</TableHead>
                                ...
                                paginatedBookings.map((booking) => (
                                    ...
                                    >
                                        ...
                                        <TableCell className="text-white max-w-[180px]">
                                            ...
                                            <p className="text-xs text-[var(--muted-text)] truncate">{booking.id}</p>
                                        </TableCell>
                                        <TableCell>
                                            {/* @ts-ignore - Supabase join typing workaround */}
                                            <p className="text-sm text-[var(--muted-text)]">{booking.properties?.owner?.full_name || "Unknown"}</p>
                                        </TableCell>
                    ...
                    {selectedBooking && (
                        <div className="space-y-5 pt-2">
                            ...
                            <div className="flex items-center gap-4 p-4 bg-[var(--card-bg)] rounded-lg border border-[var(--card-border)]">
                                ...
                                <div>
                                    <p className="text-xs text-[var(--muted-text)] uppercase tracking-widest font-bold mb-1">Guest</p>
                            ...
                            <div className="p-4 bg-[var(--card-bg)] rounded-lg border border-[var(--card-border)]">
                                <div className="flex justify-between items-start mb-3">
                                    <div>
                                        <p className="text-xs text-[var(--muted-text)] uppercase tracking-widest font-bold mb-1">Property</p>
                                        <p className="font-medium text-[var(--page-text)]">{selectedBooking.properties?.title}</p>
                                    </div>
                                    <Badge variant="outline" className="border-gold-500/30 text-gold-500">
                                        ID: {selectedBooking.id.slice(0, 8)}...
                                    </Badge>
                                </div>
                                <div className="text-sm text-[var(--muted-text)] border-t border-white/5 pt-3 mt-2 flex justify-between">
                                    <span>Host:</span>
                                    {/* @ts-ignore */}
                                    <span className="text-[var(--page-text)]">{selectedBooking.properties?.owner?.full_name || "Unknown"}</span>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between p-4 bg-[var(--card-bg)] rounded-lg border border-[var(--card-border)]">
                                <div>
                                    <p className="text-xs text-[var(--muted-text)] mb-1">Total Amount</p>
                                    <p className="text-2xl font-bold text-gold-500">{selectedBooking.total_price} DH</p>
                                </div>
