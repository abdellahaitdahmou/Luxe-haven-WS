        ...
        <div className="space-y-8 p-8 max-w-7xl mx-auto">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-[var(--muted-text)]">Manage platform revenue and host payouts.</p>
                </div>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                <Card className="bg-[var(--card-bg)] border-white/10 text-[var(--page-text)]">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-[var(--muted-text)]">Total Platform Revenue</CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <Card className="bg-[var(--card-bg)] border-white/10 text-[var(--page-text)]">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-[var(--muted-text)]">Pending Payouts</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="text-3xl font-bold text-[var(--page-text)]">{stats.pendingPayouts}</div>
                    </CardContent>
                </Card>
            </div>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="border-white/10 hover:bg-white/5">
                                ...
                                <TableHead className="text-[var(--muted-text)]">Amount</TableHead>
                                <TableHead className="text-[var(--muted-text)]">Status</TableHead>
                                <TableHead className="text-[var(--muted-text)]">Date</TableHead>
                        ...
                        <TableBody>
                            {payouts.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={5} className="text-center text-[var(--muted-text)] py-8">No payout requests found.</TableCell>
                                </TableRow>
                            ) : (
                                payouts.map((payout) => (
                                    <TableRow key={payout.id} className="border-white/10 hover:bg-white/5">
                                        <TableCell>
                                            ...
                                            <div className="text-xs text-[var(--muted-text)]">{payout.profiles?.email}</div>
                                        </TableCell>
                                        <TableCell className="font-bold">{payout.amount.toFixed(2)} DH</TableCell>
                                        <TableCell>
                                            ...
                                            {payout.status === 'pending' && (
                                                <>
                                                    <Button
                                                        ...
                                                        className="bg-green-600 hover:bg-green-700 text-[var(--page-text)]"
                                                    >
                                                    ...
                                                    <Button
                                                        ...
                                                        disabled={!!processingId}
                                                        className="bg-red-600 hover:bg-red-700 text-[var(--page-text)]"
                                                    >
